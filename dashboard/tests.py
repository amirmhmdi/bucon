...
class AllTimesheetListViewTests(TestCase):
    ...
    def test_manager_can_filter_timesheets_and_newest_same_day_entry_is_first(
        ...
        older = self.create_entry(
            "Older work", date(2026, 9, 16), start_hour=9
        )
        newer = self.create_entry(
            "Newer work", date(2026, 9, 16), start_hour=10
        )
...
class ManagerWorkflowTests(TestCase):
    ...
    def test_payroll_ignores_pending_entries(self):
        ...
        self.assertEqual(
            view._build_rows(date(2026, 9, 20), date(2026, 9, 20)),
            [],
        )
